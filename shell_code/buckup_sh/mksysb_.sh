...
(
        nim -Fo define -t mksysb -a mksysb_flags=p -a server=$SERVER -a source=$CLIENT -a mk_image=yes -a location=/export/$FS/mksysb_${CLIENT}_$DATE mksysb_${CLIENT}_$DATE
        echo $? >$RETURN
) | tee -a $LOGFILE

MK_RETURN=$(cat $RETURN)
if [ $MK_RETURN -eq 0 ]; then
...
echo "================$CLIENT SYSTEM BACKUP BIGIN at $(date)==============" | tee -a $LOGFILE
(
        nim -Fo define -t mksysb -a server=$SERVER -a source=$CLIENT -a mk_image=yes -a location=/export/$FS/mksysb_${CLIENT}_$DATE mksysb_${CLIENT}_$DATE
        echo $? >$RETURN
) | tee -a $LOGFILE
