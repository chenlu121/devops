...
if [  $? -eq 0 ]
then
cat $DIR/hosts.txt | uniq | grep -v "#" | grep -v  '^$' | awk '{print $2}' | while read LINE
do
...
{
IEEE_ENT=
while getopts n:t:a:s:c:o:i:g: option
do
        case $option in
                n) NAME=$OPTARG;;
                t) TYPE=$OPTARG;;
...
add_route()
 {
 while getopts o:O:d:D:F: option
 do
         case $option in
                 o) ORIGIN=$OPTARG;;
                 O) OGATEWAY=$OPTARG;;
...
grep -v "#" $DIR/hosts.txt | grep -v ^$ | awk '{print $1}' |cut -d. -f 1,2,3 | sort -u |  while read line
do
...
do
        ...
        read C_temp
        case $C_temp in
        (Y) echo "nim install start";break;;
        (N) exit 0;;
